...
% Define subtle link colors (professional & printer-friendly)
\definecolor{InternalLink}{HTML}{1A4E80} % dark desaturated blue
\definecolor{URLLink}{HTML}{7A1A80}      % purple-ish, distinct but subtle

\usepackage[
    pdfencoding=auto,
    psdextra,
    colorlinks=true,
    linkcolor=InternalLink,  % internal links (sections, equations, refs)
    citecolor=InternalLink,  % citations
    filecolor=URLLink,       % file links (rare)
    urlcolor=URLLink,        % external URLs
    menucolor=InternalLink,
    anchorcolor=InternalLink,
]{hyperref}

% Optional: makes URLs nicer (line breaks, no ugly fonts)
\usepackage{url}
\urlstyle{same}
...
\begin{document}
    ...
    If the sum of percentages is below 100\%, the residual is sent to the notary address to prevent funds from being stranded forever and can be distributed fairly and according to local law.
    ...
    Additionally, \texttt{updateState()} could be automated using off-chain keepers (e.g., Chainlink Automation\footnotemark) to guarantee timely transitions without relying on manual calls \parencite{chainlink-automation}.
    ...
    \footnotetext{For documentation refer to: https://docs.chain.link/chainlink-automation}
    ...
    \begin{itemize}
        ...
        Furthermore even a compromised notary could not trigger payout while the owner still checks in.
    ...
    \begin{enumerate}
        \item \textbf{Off-chain automation (Chainlink Keepers):} Use a keeper network to call \newline
        \texttt{releaseVestedFunds()} at scheduled intervals \parencite{chainlink-automation}.
    ...
    \begin{appendices}
        ...
        The entire project can be found at the \href{https://github.com/vincentschall/decentralized_inheritance_protocol}{Project repository}

        \section{Contract-Code}\label{sec:contract-code}
